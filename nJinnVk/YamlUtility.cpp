...
namespace YAML {
	Node convert<vk::ShaderStageFlagBits>::encode(const vk::ShaderStageFlagBits& rhs) {
		Node node;
		const char * string;
		switch (uint32_t(rhs)) {
	...
	bool convert<vk::ShaderStageFlagBits>::decode(const Node& node, vk::ShaderStageFlagBits& rhs) {
	...
	Node convert<vk::CompareOp>::encode(const vk::CompareOp& rhs) {
		Node node;
		const char * string;
		switch (uint32_t(rhs)) {
	...
	bool convert<vk::CompareOp>::decode(const Node& node, vk::CompareOp& rhs) {
