...
namespace nJinn {
	DescriptorWriter::DescriptorWriter(const DescriptorSet & set) :
	...
	DescriptorWriter & DescriptorWriter::attachment(vk::DescriptorImageInfo * imageInfos, uint32_t binding, uint32_t count, uint32_t baseIndex)
	{
	...
	DescriptorWriter & DescriptorWriter::uniformBuffer(class UniformBuffer * uniformBuffer, uint32_t binding, uint32_t count, uint32_t baseIndex)
	{
		for (uint32_t i = 0; i < count; ++i) {
	...
	DescriptorWriter & DescriptorWriter::uniformBuffer(vk::Buffer buffer, vk::DeviceSize offset, vk::DeviceSize size, uint32_t binding, uint32_t count, uint32_t baseIndex)
	{
		for (uint32_t i = 0; i < count; ++i) {
